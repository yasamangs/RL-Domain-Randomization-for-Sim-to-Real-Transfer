...
def test(args, test_env, seed):
    ...
    if args.render:
        imageio.mimwrite(
            f'{args.directory}/SAC-({args.train_env} to {args.test_env})-test.gif', frames, fps=30)
...
def main():
    ...
    if args.train:
        ...
        for metric, records in zip(('reward', 'length'), (pool['rewards'], pool['lengths'])):
            ...
            if metric == 'reward':
                path = os.path.join(
                    args.directory, f'SAC-({args.train_env} to {args.test_env})-rewards.npy')
                np.save(path, ys)
            track(metric, xs, ys, sigmas, args,
                  label='SAC',
                  filename=f'SAC-({args.train_env} to {args.test_env})-{metric}')
        print(
        ...
        agent.save(
            f'{args.directory}/SAC-({args.train_env} to {args.test_env}).mdl')
        print(
