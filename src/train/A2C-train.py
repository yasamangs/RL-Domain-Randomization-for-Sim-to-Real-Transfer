...
def test(args, test_env, seed):
    ...
    if args.render:
        imageio.mimwrite(
            f'{args.directory}/A2C-{args.batch_size}-({args.train_env} to {args.test_env})-test.gif', frames, fps=30)
...
def arrange(args, stacks, train_env):
    ...
    ), f'{args.directory}/A2C-{args.batch_size}-({args.train_env} to {args.test_env}).mdl')
    print(
...
def main():
    ...
    if args.train:
        ...
        for metric, records in zip(('reward', 'length'), (pool['rewards'], pool['lengths'])):
            ...
            if metric == 'reward':
                path = os.path.join(
                    args.directory, f'A2C-{args.batch_size}-({args.train_env} to {args.test_env})-rewards.npy')
                np.save(path, ys)
